...
module.exports = {
  printResourceReport: function (page) {
    console.log('--------speedgun.reportData.resources',speedgun.reportData.resources)
    if (speedgun.reportData) {
        ...
        if (headers) {
          ...
          if (entry && speedGunArgs.cdnDebug) {
            ...
            response[entry].headers.forEach(function (header) {
              ...
              if (header.name.toLowerCase().indexOf('akamai') > -1) {
                reportResource.AKAMAI.active = true
              }
              if (header.name.toLowerCase().indexOf('fastly') > -1) {
                reportResource.FASTLY.active = true
              }
              if (header.name.toLowerCase().indexOf('instart') > -1) {
                reportResource.INSTART.active = true
              }
              if (header.value.toLowerCase().indexOf('netdna') > -1) {
                reportResource.MAXCDN.active = true
              }
              if (header.value.toLowerCase().indexOf('cloudfront') > -1) {
                reportResource.CLOUDFRONT.active = true
              }
              
              
              //akamai rules
              if (header.name.toLowerCase() === 'x-akamai-transformed') {
                reportResource.AKAMAI.features.push({feature: "FEO_ENABLED"});
              }
              if (header.name.toLowerCase() === 'x-akamai-edgescape') {
                reportResource.AKAMAI.features.push({feature: "CONTENT_TARGETING"});
              } // https://community.akamai.com/community/web-performance/blog/2016/03/16/content-targeting-a-basic-introduction
              if (header.value.toLowerCase().indexOf('pmb=mrum') > -1) {
                reportResource.AKAMAI.features.push({feature: "RUM_ENABLED"});
              }
              ...
              if (header.name.toLowerCase() === ('x-check-cacheable' && header.value.indexOf('NO') > -1)) {
                reportResource.GENERIC.features.push({feature: "NOT_CACHED"});
              }
              if (header.name.toLowerCase() === ('x-cache' && header.value.indexOf('HIT') > -1)) {
                reportResource.GENERIC.features.push({feature: "CACHED"});
              }
              
              //instart rules
              if (header.name === 'x-instart-cache-id') {
                reportResource.INSTART.features.push({feature: "CACHED"});
              }
              if (header.name === 'X-Instart-Streaming') {
                if (header.value.indexOf('HtmlStreaming:HIT;InstantLoad:HIT') > -1) {
                  reportResource.INSTART.features.push({feature: "HTML Streaming and InstantLoad are both enabled"});
                }
                if (header.value.indexOf('HtmlStreaming:HIT;InstantLoad:SKIP,optimization_disabled') > -1) {
                  reportResource.INSTART.features.push({feature: "HTML Streaming is enabled, InstantLoad is disabled"});
                }
                if (header.value.indexOf('HtmlStreaming:MISS,optimization_disabled;InstantLoad:HIT') > -1) {
                  reportResource.INSTART.features.push({feature: "HTML Streaming is disabled, Instant Load is enabled"});
                }
                if (header.value.indexOf('HtmlStreaming:MISS,reloaded_request;InstantLoad:SKIP,optimization_disabled') > -1) {
                  reportResource.INSTART.features.push({feature: "HTML Streaming is enabled but was not applied because the request was reloaded; InstantLoad is disabled"});
                }
                if (header.value.indexOf('HtmlStreaming:MISS,streaming_cache_not_ready;InstantLoad:SKIP,html_streaming_miss') > -1) {
                  reportResource.INSTART.features.push({feature: "HTML Streaming is enabled but was not applied because the streaming cache was not yet ready; InstantLoad is disabled"});
                }
                if (header.value.indexOf('js_profiled') > -1) {
                  reportResource.INSTART.features.push({feature: "JavaScript Streaming is enabled"});
                }
                if (header.value.indexOf('js_profiled') > -1) {
                  reportResource.INSTART.features.push({feature: "JavaScript Streaming is enabled and the optimized code was delivered to the client"});
                }
              }
          ...
          reportResource.imageSize = {"width": 0, "height": 0};
          try { //if we get an image content type, evaluate the page and look for more data, e.g. width/height
            reportResource.headers.forEach(function (header) {
              ...
              if (header.value.indexOf('image/') > -1) {
                ...
                fpi.writeLine(reportResource.url);
              } else {
                //reportResource.imageSize = undefined;
              }
            });
          } catch (e) {
            console.log('!!!problem with iterating headers')
          }
          ...
          try {
            ...
            console.log('!!!problem with iterating headers 2')
          }
          ...
          try {
            ...
            console.log('!!problem with reporting rules')
          }
